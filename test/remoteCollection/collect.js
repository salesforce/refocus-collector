...
describe('test/remoteCollection/collect.js >', () => {
  ...
  describe('generatorsGlobal shared >', () => {
    ...
    it('collecting data from argus, generatorsGlobal updated', (done) => {
      const remoteUrl = 'https://xyztest.argusTest.com';
      const generator = {
        name: 'Generator0',
        intervalSecs: 1,
      ...
      const token = {
        // eslint-disable-next-line camelcase
        access_token: 'some-random-token-1',
      };
      ...
      collect.doCollect(generator)
        .then((collectRes) => {
          ...
          expect(collectRes.subjects).to.deep.equal(generator.subjects);
          expect(collectRes.res.request.header.Authorization)
            .to.equal('Bearer some-random-token-1');

          nock(remoteUrl)
            .post('/argusws/v2/auth/login')
            .reply(httpStatus.OK, { token: 'some-random-token-2' },
              { 'Content-Type': 'application/json' });
          ...
          return collect.doCollect(generator);
        })
        .then((cRes) => {
          expect(cRes.res.request.header.Authorization)
            .to.equal('Bearer some-random-token-1'); // original token is used
          done();
        })
        .catch(done);
    });
