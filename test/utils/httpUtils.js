...
describe('test/utils/httpUtils.js >', () => {
  ...
  describe('doPost >', () => {
    ...
    it('post ok, returns 429 error but retries and succeeds', (done) => {
      ...
      const responseHeader = { 'Retry-After': .2 };
      const reregisterEndpoint = `/v1/collectors/${collectorName}/reregister`;
      nock(refocusUrl, {
        reqheaders: { authorization: dummyToken },
      })
  ...
  describe('doBulkUpsert >', () => {
    ...
    it('arr arg is not an array', (done) => {
      httpUtils.doBulkUpsert(refocusUrl, dummyUserToken, intervalSecs,
        null, 'Hi')
        .then((res) =>
          expect(res).to.have.property('name', 'ValidationError'))
        .then(() => done())
    ...
    it('drop request because retrying for too long', (done) => {
      ...
      httpUtils.doBulkUpsert(refocusUrl, dummyUserToken,
        ...
        .then((res) => {
          ...
          expect(res).to
            .equal(`Request dropped after ${timeSpent} milliseconds`);
          done();
        })
        .catch(done);
  ...
  describe('validateSubjectQuery', () => {
    it('empty query string', () => {
      try {
        httpUtils.validateSubjectQuery('');
      ...
      try {
        httpUtils.validateSubjectQuery(null);
        throw new Error('expecting error');
      } catch (err) {
        expect(err).to.have.property('name', 'ValidationError');
        expect(err).to.have.property('message', 'Missing subject query');
      ...
      try {
        httpUtils.validateSubjectQuery(undefined);
        throw new Error('expecting error');
      } catch (err) {
        expect(err).to.have.property('name', 'ValidationError');
        expect(err).to.have.property('message', 'Missing subject query');
    ...
    it('starts with ?', () => {
      const q = httpUtils.validateSubjectQuery('a=b&c=d');
  ...
  describe('getSubjectsForGenerator >', () => {
    ...
    it('ok, query starts with "?"', (done) => {
      nock(refocusUrl, {
        ...
        .query({ absolutePath: 'NorthAmerica.Canada', isPublished: true })
